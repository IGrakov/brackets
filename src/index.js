module.exports = function check(str, bracketsConfig) {
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        // check if such bracket is already in the stack and
        // then current one is not an opening bracket
        if (bracketsOpening.includes(str[i]) && (!bracketsStack.includes(str[i]) || !bracketsClosing.includes(str[i]))) {
            ...
            if (bracketsStack[bracketsStack.length - 1] === bracketsOpening[bracketsClosing.indexOf(str[i])]) {
                bracketsStack.pop();
            } else {
                return false;
            }
